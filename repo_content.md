...
This document describes the new workflow blocks functionality that allows users to create draggable workflow components with AI prompts on a timeline.

## Features Implemented

### 1. Enhanced WorkflowBox Component
- **Prompt Field**: Each workflow box now includes a text field for AI prompts
- **Inline Editing**: Users can edit prompts directly in the workflow box or through the settings modal
- **Visual Feedback**: Clear indication when prompts are set or missing

### 2. Custom AI Task Block
- **New Block Type**: Added "Custom AI Task" as a general-purpose workflow block
- **Flexible Prompting**: Users can create custom AI tasks with their own prompts
- **Zap Icon**: Uses a lightning bolt icon to represent the custom AI functionality

### 3. Backend Integration
- **API Endpoint**: `/api/workflows/[workflow_id]/blocks`
- **Database Schema**: New `blocks` table in Supabase
- **Full CRUD Operations**: Create, read, update, and delete blocks
- **Authentication**: Secure API endpoints with user authentication

### 4. Timeline Integration
- **Save Changes Button**: Saves all blocks to the database with their positions and prompts
- **Real-time Updates**: Prompt changes are immediately reflected in the UI
- **Position Tracking**: Each block stores its offset (hours before/after meeting)

## Database Schema

The `blocks` table includes:
- `id`: UUID primary key
- `workflow_id`: References the workflow
- `user_id`: References the authenticated user
- `type`: Block type (e.g., "Email Collector", "Custom AI Task")
- `prompt`: AI prompt text for the block
- `offset`: Time offset in hours relative to meeting (negative = before, positive = after)
- `created_at` / `updated_at`: Timestamps

## API Endpoints

### POST `/api/workflows/[workflow_id]/blocks`
Saves workflow blocks to the database.
```json
{
  "blocks": [
    {
      "type": "Custom AI Task",
      "prompt": "Generate a summary of key discussion points",
      "offset": -2.0
    }
  ]
}
...
### GET `/api/workflows/[workflow_id]/blocks`
Retrieves all blocks for a workflow, ordered by offset.

## Usage

1. **Drag Blocks**: Drag workflow components from the sidebar to the timeline
2. **Set Prompts**: Click the edit icon to add AI prompts for each block
3. **Position Blocks**: Drop blocks on timeline positions (meeting time +/- hours)
4. **Save Workflow**: Click "Save Changes" to persist the workflow to the database

## Security

- Row Level Security (RLS) enabled on the blocks table
...
- `WorkflowBox.jsx`: Added prompt editing functionality
- `WorkflowSidebar.jsx`: Added custom AI task block
- `Timeline.jsx`: Added save functionality and prompt updates
- `WorkflowSettingsModal.jsx`: Added prompt field to settings
- `route.js`: New API endpoint for blocks CRUD operations
- `database-schema.sql`: Database schema for blocks table
...
3. Verify the save changes button works correctly
4. Test prompt editing in both inline and modal modes
